...
export default function DataTabled({
    ...
    const changeStatus = (i) => {
        ...
        updateService(newServices[i].id, {
            status: newServices[i].status,
        }).then(
            (res) => {
            ...
            <React.Fragment>
                <div className="d-flex flex-row align-items-center justify-content-center">
                    ...
                    {/* <i
                        ...
                        onClick={() =>
                            deleteServiceData(rowData.id, serviceNature)
                        }
                    ></i> */}
                </div>
            </React.Fragment>
    ...
    console.log("carlistLength", carlistLength.length < 0, carlistLength.length);
    return (
        <div className="service-datatable">
            <div style={{ height: "10%" }} className="service-datatable-header">
                ...
                {
                    carlistLength.length > 0 &&
                    <button
                        className="btn-white w-25 ms-auto"
                        onClick={() => openModal(serviceNature)}
                    >
                        {t('Add')} +
                    </button>
                }
            ...
            >
                ...
                <Column field="timetaken" header={t('Time Taken')}></Column>
                <Column
                    field="id"
                    header={t('Status')}
                    body={statusTemplate}
